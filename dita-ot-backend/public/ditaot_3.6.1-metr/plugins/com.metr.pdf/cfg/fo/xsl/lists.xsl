...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="*[contains(@class, ' topic/ul ')]">
        ...
        <fo:list-block xsl:use-attribute-sets="ul">
            ...
            <xsl:if test="ancestor::*[contains(@class, ' topic/row ')]">
                <xsl:attribute name="provisional-distance-between-starts">3mm</xsl:attribute>
                <xsl:attribute
                    name="margin-left">1pt</xsl:attribute>
            </xsl:if>
            <xsl:apply-templates />
        </fo:list-block>
        <xsl:apply-templates
    ...
    <xsl:template
        match="*[contains(@class, ' topic/ul ')][empty(*[contains(@class, ' topic/li ')])]"
        priority="10" />

    <!-- Template for ordered lists (ol) -->
    ...
    <!-- Template for nested ordered lists (ol) within another ordered list -->
    <xsl:template
        match="*[contains(@class, ' topic/ol ')][ancestor::*[contains(@class, ' topic/ol ')]]"
        priority="2">
        <xsl:apply-templates select="*[contains(@class,' ditaot-d/ditaval-startprop ')]"
            mode="outofline" />
    <fo:list-block xsl:use-attribute-sets="ol"
            provisional-distance-between-starts="3mm" provisional-label-separation="9mm">
            <xsl:call-template name="commonattributes" />
            <xsl:apply-templates />
        </fo:list-block>
    <xsl:apply-templates
    ...
    <xsl:template
        match="*[contains(@class, ' topic/ol ')][empty(*[contains(@class, ' topic/li ')])]"
        priority="10" />
        ...
        <xsl:choose>
            <xsl:when test="$depth = 2">
                <fo:list-item xsl:use-attribute-sets="ul.li">
                    <xsl:call-template name="commonattributes" />
                    <xsl:choose>
                        <xsl:when test="ancestor::*[contains(@class, ' topic/row ')]">
                            <xsl:attribute name="margin-left">1pt</xsl:attribute>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:attribute name="margin-left">-2.1mm</xsl:attribute>
                        </xsl:otherwise>
                    </xsl:choose>

                    <fo:list-item-label xsl:use-attribute-sets="ul.li__label">
                        <fo:block xsl:use-attribute-sets="ul.li__label__content" font-size="10pt">
                    ...
                    <fo:list-item-body xsl:use-attribute-sets="ul.li__body">
                        <fo:block xsl:use-attribute-sets="ul.li__content">
                            <xsl:if
                                test="descendant::*[contains(@class,' topic/image ') and not(parent::*[contains(@class,' topic/fig ')])]">
            ...
            <xsl:when test="$depth = 1">
                <fo:list-item xsl:use-attribute-sets="ol.li">
                    ...
                    <fo:list-item-label xsl:use-attribute-sets="ol.li__label">
                        <fo:block xsl:use-attribute-sets="ol.li__label__content" text-align="right">
                            <xsl:call-template name="getVariable">
                                <xsl:with-param name="id"
                                    select="concat('Ordered List Number ', $depth)" />
    ...
    <xsl:template
        match="*[contains(@class, ' topic/sl ')][empty(*[contains(@class, ' topic/sli ')])]"
        priority="10" />
